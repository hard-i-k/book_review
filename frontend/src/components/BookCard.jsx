...
const BookCard = ({ book, onDelete }) => {
  ...
  return (
    <div
      className="bg-white/90 dark:bg-white/10 backdrop-blur-md rounded-xl shadow-2xl p-6 flex flex-col items-center border-t-4 border-purple-700/60 hover:shadow-3xl transition-all duration-200 relative cursor-pointer"
    ...
    >
      ...
      <p className="text-gray-700 dark:text-purple-200 text-sm mb-2 line-clamp-3">
        ...
        {book.description.length > 120 && (
          <button onClick={e => { e.stopPropagation(); setShowModal(true); }} className="ml-2 text-purple-500 underline hover:text-purple-700">Read more</button>
        )}
      </p>
      <div className="flex gap-2 w-full justify-center">
        <Link
          to={`/books/${book._id}`}
          className="bg-gradient-to-r from-purple-600 to-blue-500 text-white px-4 py-2 rounded-lg font-semibold shadow hover:scale-105 hover:from-blue-500 hover:to-purple-600 transition-all duration-200"
        >
          Review
        </Link>
        {isOwner && (
          <button
            ...
            className="bg-red-500 text-white px-4 py-2 rounded-lg font-semibold shadow hover:bg-red-700 transition-all duration-200"
          >
            Delete
          </button>
        )}
